...
from full_pyastec import pymdb
...
def display_family_names(base):
    ...
    for i in range(1-full_pyas.odessa_shift(), full_pyas.odbase_family_number(base)+1-full_pyas.odessa_shift()):
...
# Interaction with base
    ...
    def get_index(self, circuit, family, name):
        result = -1
        for i in range(1-full_pyas.odessa_shift(), full_pyas.odbase_size( circuit, family )+1-full_pyas.odessa_shift()):
            sub=full_pyas.odbase_get_odbase( circuit, family, i )
            if( full_pyas.odbase_size( sub, "NAME" ) > 0 ):
                a_name = full_pyas.odbase_get_string( sub, "NAME", 1-full_pyas.odessa_shift() )
                if( a_name == name ) :
                    result = i
                    break
        if( result == -1 ):
    ...
    def tag(self, circuit_name, family, name, dummy_arg):
        if circuit_name is None:
            ...
            circuit = full_pyas.odbase_get_odbase( pyas.root_database(), circuit_name, 1-full_pyas.odessa_shift() )
        index = self.get_index( circuit, family, name )
        vol = full_pyas.odbase_get_odbase( circuit, family, index )
        full_pyas.odbase_put_int( vol, "INDEX", index, 1-full_pyas.odessa_shift() )
    ...
    def hide(self, circuit_name, family, name, dummy_arg):
        ...
        index = self.get_index( circuit, family, name )
        vol = full_pyas.odbase_get_odbase( circuit, family, index )
        new_index = full_pyas.odbase_size( circuit, "H"+family )
        full_pyas.odbase_insert_odbase(circuit,"H"+family, vol, new_index+1-full_pyas.odessa_shift())
        full_pyas.odbase_delete_element(circuit,family, index)
        self.log( "Hiding "+family+" named "+name)

    def hide_link_in_family(self, circuit, family, link, name):
        for i in range(1-full_pyas.odessa_shift(), full_pyas.odbase_size( circuit, family )+1-full_pyas.odessa_shift() ):
            vol = full_pyas.odbase_get_odbase( circuit, family, i )
            for j in range(1-full_pyas.odessa_shift(), full_pyas.odbase_size( vol, link )+1-full_pyas.odessa_shift() ):
                jun = full_pyas.odbase_get_string( vol, link, j )
                if( jun == name ):
                    full_pyas.odbase_delete_element(vol, link, j)
                    full_pyas.odbase_insert_string(vol, "H"+link, name, 1-full_pyas.odessa_shift())
                    self.log( "Hiding in "+family+" link " +link+" to "+name)

    def restore(self, circuit, family):
        N = full_pyas.odbase_size( circuit, "H"+family )
        indexing=list()
        for i in range(1-full_pyas.odessa_shift(), N+1-full_pyas.odessa_shift() ):
            obj=full_pyas.odbase_get_odbase(circuit, "H"+family, i)
            # new_index = full_pyas.odbase_size( circuit, family )
            new_index = full_pyas.odbase_get_int( obj, "INDEX", 1-full_pyas.odessa_shift() )
            indexing.append([new_index, obj])
        sorted_list=sorted(indexing, key=lambda pair: pair[0])
        for index,obj in sorted_list:
            full_pyas.odbase_insert_odbase(circuit,family, obj, index)
            name = full_pyas.odbase_get_string(obj, "NAME", 1-full_pyas.odessa_shift())
            self.log( "Restoring "+family+" named "+name+" at "+str(index))
        for i in range( N-full_pyas.odessa_shift(),-full_pyas.odessa_shift(),-1 ):
            full_pyas.odbase_delete_element(circuit, "H"+family, i)
    ...
    def compute_flow( self, circuit, alter_jun_name, spec, vol_name, alter_vol_name ):
        ...
        jun = full_pyas.odbase_get_odbase( circuit, "JUNCTION", self.get_index( circuit, "JUNCTION", alter_jun_name ) )
        jun_geom = full_pyas.odbase_get_odbase( jun, "GEOM", 1-full_pyas.odessa_shift() )
        S = full_pyas.odbase_get_double( jun_geom, "S", 1-full_pyas.odessa_shift() )
        if(spec=='WATER'):
            post="liq"
        else:
            post="gas"
        jun_ther = full_pyas.odbase_get_odbase( jun, "THER", 1-full_pyas.odessa_shift() )
        Qr1 = full_pyas.odbase_get_odr1( jun_ther, dico_massic_flow[spec], 1-full_pyas.odessa_shift() )
        Q = full_pyas.odr1_get( Qr1, 2-full_pyas.odessa_shift() )
        upstream_vol = alter_vol_name
        if( Q < 0. ) : upstream_vol = vol_name
        donnor_vol = full_pyas.odbase_get_odbase( circuit, "VOLUME", self.get_index( circuit, "VOLUME", upstream_vol )  )
        donnor_vol_ther = full_pyas.odbase_get_odbase( donnor_vol, "THER", 1-full_pyas.odessa_shift() )
        Tr1 = full_pyas.odbase_get_odr1( donnor_vol_ther, "T_"+post, 1-full_pyas.odessa_shift() )
        T = full_pyas.odr1_get( Tr1, 2-full_pyas.odessa_shift() )
        Pr1 = full_pyas.odbase_get_odr1( donnor_vol_ther, "P", 1-full_pyas.odessa_shift() )
        P = full_pyas.odr1_get( Pr1, 2-full_pyas.odessa_shift() )

        return [ Q, T, P ]

    def get( self, base, family:str, name:str ):
        i = self.get_index( base, family, name )
        return full_pyas.odbase_get_odbase( base,  family, i )

    def transfer_from_vol_ther_to_connecti_ther( self, volume_ther, connecti_ther, key:str ):

        tab = full_pyas.odbase_get_odr1( volume_ther, key, 1-full_pyas.odessa_shift() )
        full_pyas.odbase_put_odr1( connecti_ther, key, tab, 1-full_pyas.odessa_shift() )
        self.log("Put "+key+" = "+str(full_pyas.odr1_get( tab, 2-full_pyas.odessa_shift() )))

    def open_and_feed_connecti(self, conn, init):


        connecti = self.get( pyas.root_database(), "CONNECTI", conn )
        full_pyas.odbase_put_string( connecti, 'STAT', 'ON', 1-full_pyas.odessa_shift() )

        vol_name = full_pyas.odbase_get_string( connecti, "VOLUME", 1-full_pyas.odessa_shift() )
        circuit_name = full_pyas.odbase_get_string( connecti, "TO", 1-full_pyas.odessa_shift() )
        if( circuit_name=='USER' ): circuit_name = full_pyas.odbase_get_string( connecti, "FROM", 1-full_pyas.odessa_shift() )
        circuit = full_pyas.odbase_get_odbase( pyas.root_database(), circuit_name, 1-full_pyas.odessa_shift() )

        private = full_pyas.odbase_get_odbase( connecti, "PRIVATE", 1-full_pyas.odessa_shift() )
        alter_vol_name = full_pyas.odbase_get_string( private, "VOLUME", 1-full_pyas.odessa_shift() )
        alter_jun_name = full_pyas.odbase_get_string( private, "JUNCTION", 1-full_pyas.odessa_shift() )
        connecti_type = full_pyas.odbase_get_string( connecti, "TYPE", 1-full_pyas.odessa_shift() )

        self.log( "\n Open and feed "+conn+" of type "+ connecti_type )

        # Connecti of type SOURCE
        if( connecti_type=="SOURCE" ):
                for i in range(1-full_pyas.odessa_shift(), full_pyas.odbase_size( connecti, "SOURCE" )+1-full_pyas.odessa_shift() ):
                    source = full_pyas.odbase_get_odbase( connecti, "SOURCE", i )
                    spec = full_pyas.odbase_get_string( source, "SPEC", 1-full_pyas.odessa_shift() )
                    Q, T, P = self.compute_flow( circuit, alter_jun_name, spec, vol_name, alter_vol_name )
                    if init:
                        tab = full_pyas.odr1_init()
                        full_pyas.odr1_put( tab, 1-full_pyas.odessa_shift(), self.time_beg )
                        full_pyas.odr1_put( tab, 2-full_pyas.odessa_shift(), Q )
                        full_pyas.odr1_put( tab, 3-full_pyas.odessa_shift(), T )
                        full_pyas.odr1_put( tab, 4-full_pyas.odessa_shift(), P )
                        full_pyas.odbase_put_odr1( source, "FLOW", tab, 1-full_pyas.odessa_shift() )
                    else:
                        tab = full_pyas.odbase_get_odr1( source, "FLOW", 1-full_pyas.odessa_shift() )
                        full_pyas.odr1_put( tab, 5-full_pyas.odessa_shift(), self.time_end )
                        full_pyas.odr1_put( tab, 6-full_pyas.odessa_shift(), Q )
                        full_pyas.odr1_put( tab, 7-full_pyas.odessa_shift(), T )
                        full_pyas.odr1_put( tab, 8-full_pyas.odessa_shift(), P )
                    full_pyas.odbase_put_odr1( source, "FLOW", tab, 1-full_pyas.odessa_shift() )

        elif( connecti_type=="BCPRESS" ):
            junction = self.get( circuit, "JUNCTION", alter_jun_name )
            junction_ther = full_pyas.odbase_get_odbase( junction, "THER", 1-full_pyas.odessa_shift() )
            volume = self.get( circuit, "VOLUME", alter_vol_name )
            volume_ther = full_pyas.odbase_get_odbase( volume, "THER", 1-full_pyas.odessa_shift() )
            connecti_ther = full_pyas.odbase_get_odbase( connecti, "THER", 1-full_pyas.odessa_shift() )
    ...
    def transfer_from_connecti_ther_to_junction_ther( self, connecti_ther, junction_ther, key ):
        ...
        tab = full_pyas.odbase_get_odr1( connecti_ther, key, 1-full_pyas.odessa_shift() )
        full_pyas.odbase_put_odr1( junction_ther, key, tab, 1-full_pyas.odessa_shift() )
        self.log("Get "+key+" = "+str(full_pyas.odr1_get( tab, 2-full_pyas.odessa_shift() )))

    def close_connecti(self, conn):

        self.log( "\n Close "+conn )
        connecti = self.get( pyas.root_database(), "CONNECTI", conn )
        full_pyas.odbase_put_string( connecti, 'STAT', 'OFF', 1-full_pyas.odessa_shift() )
        connecti_type = full_pyas.odbase_get_string( connecti, "TYPE", 1-full_pyas.odessa_shift() )
    ...
    def extract_data(self, source, dest ):
        ...
        for i in range(1-full_pyas.odessa_shift(), full_pyas.odbase_family_number(source)+1-full_pyas.odessa_shift() ):
            fname = full_pyas.odbase_name(source,i).strip()
            ftyp = full_pyas.odbase_type(source,fname)
            fnum = full_pyas.odbase_size(source,fname)

            for i in range(1-full_pyas.odessa_shift(), fnum+1-full_pyas.odessa_shift()):
                    if ( ftyp==full_pyas.od_base ):
                        sub_source=full_pyas.odbase_get_odbase( source, fname, i )
                        sub_dest=full_pyas.odbase_get_odbase( dest, fname, i )
                        self.extract_data(sub_source, sub_dest)
                    elif( ftyp==full_pyas.od_r0 ):
                        full_pyas.odbase_put_double(dest,fname,full_pyas.odbase_get_double(source,fname,i),i)
                    elif( ftyp==full_pyas.od_i0 ):
                        full_pyas.odbase_put_int(dest,fname,full_pyas.odbase_get_int(source,fname,i),i)
                    elif( ftyp==full_pyas.od_r1 ):
                        full_pyas.odbase_put_odr1(dest,fname,full_pyas.odbase_get_odr1(source,fname,i),i)
                    elif( ftyp==full_pyas.od_c0 ):
                        full_pyas.odbase_put_string(dest,fname,full_pyas.odbase_get_string(source,fname,i),i)
                    elif( ftyp==full_pyas.od_t ):
                        full_pyas.odbase_put_odt(dest,fname,full_pyas.odbase_get_odt(source,fname,i),i)
                    else:
    ...
    def extract(self, circuit_name, family, name, reduced):
        ...
        if circuit_name is None:
            ...
            circuit = full_pyas.odbase_get_odbase( pyas.root_database(), circuit_name, 1-full_pyas.odessa_shift() )
            reduced_circuit = full_pyas.odbase_get_odbase( reduced, circuit_name, 1-full_pyas.odessa_shift() )
        vol = full_pyas.odbase_get_odbase( circuit, family, self.get_index( circuit, family, name ) )
        reduced_vol = full_pyas.odbase_get_odbase( reduced_circuit, family, self.get_index( reduced_circuit, family, name ) )
        self.extract_data(reduced_vol,vol)

    def put_values(self,base,time,values):
        for v in values:
            tab=full_pyas.odr1_init()
            full_pyas.odr1_put(tab,1-full_pyas.odessa_shift(),time)
            full_pyas.odr1_put(tab,2-full_pyas.odessa_shift(),values[v])
            full_pyas.odbase_put_odr1(base,v,tab,1-full_pyas.odessa_shift() )

    def get_values(self,base):
        r={}
        for i in range(1-full_pyas.odessa_shift(), full_pyas.odbase_family_number(base)+1-full_pyas.odessa_shift() ):
            fname = full_pyas.odbase_name(base,i).strip()
            ftyp = full_pyas.odbase_type(base,fname)
            fnum = full_pyas.odbase_size(base,fname)
            if( ftyp==full_pyas.od_r0 ):
                r[fname]=full_pyas.odbase_get_double(base,fname,1-full_pyas.odessa_shift())
            elif( ftyp==full_pyas.od_r1 ):
                r[fname]=full_pyas.odr1_get( full_pyas.odbase_get_odr1(base,fname,1-full_pyas.odessa_shift()), 2-full_pyas.odessa_shift() )
            else:
    ...
    def complete_missing_data_from_metamodelled_ones(self, circuit_name, family, name, dummy_arg):
        x = full_pyas.new_doublep()
        R=8.314
        pymdb.get('H2O','M',x)
        Mh2o=full_pyas.doublep_value(x)
        pymdb.get('H2','M',x)
        Mh2=full_pyas.doublep_value(x)
        self.log("Complete "+name+" in family "+family+" of circuit "+str(circuit_name))
        if circuit_name is None:
            circuit=pyas.root_database()
        else:
            circuit = full_pyas.odbase_get_odbase( pyas.root_database(), circuit_name, 1-full_pyas.odessa_shift() )
        obj = full_pyas.odbase_get_odbase( circuit, family, self.get_index( circuit, family, name ) )
        if family=="VOLUME":
            vol = obj
            computed={}
            geom = full_pyas.odbase_get_odbase( vol, "GEOM", 1-full_pyas.odessa_shift() )
            volume = full_pyas.odbase_get_double( geom, "V", 1-full_pyas.odessa_shift() )

            ther = full_pyas.odbase_get_odbase( vol, "THER", 1-full_pyas.odessa_shift() )
            v = self.get_values( ther )

            computed['P'] = v['P_h2']+v['P_steam']

            pymdb.get('H2O','P_sat(T)',v['T_liq'],x)
            computed['Psat']=full_pyas.doublep_value(x)

            pymdb.get('H2O','rho_l(T,P)',v['T_liq'],computed['P'],x)
            computed['rho_liq']=full_pyas.doublep_value(x)

            pymdb.get('H2O','T_sat(P)',v['P_steam'],x) # To check what pressure should be used
            computed['T_sat']=full_pyas.doublep_value(x)
            ...
            Vl_bar = 0.
            Vg_bar = 0.
            N = full_pyas.odbase_size(vol,'JUNCTION')
            for j in range(1-full_pyas.odessa_shift(), N+1-full_pyas.odessa_shift() ):
                jname = full_pyas.odbase_get_string(vol,'JUNCTION',j)
                junc = full_pyas.odbase_get_odbase( circuit, "JUNCTION", self.get_index( circuit, "JUNCTION", jname ) )
                juncther = full_pyas.odbase_get_odbase( junc, "THER", 1-full_pyas.odessa_shift() )
                Vgr1 = full_pyas.odbase_get_odr1( juncther, "v_gas", 1-full_pyas.odessa_shift() )
                Vg = full_pyas.odr1_get( Vgr1, 2-full_pyas.odessa_shift() )
                Vlr1 = full_pyas.odbase_get_odr1( juncther, "v_liq", 1-full_pyas.odessa_shift() )
                Vl = full_pyas.odr1_get( Vlr1, 2-full_pyas.odessa_shift() )
                Vl_bar = Vl_bar + Vl / N
                Vg_bar = Vg_bar + Vg / N
        ...
        if family=="JUNCTION":
            computed={}
            jun = obj
            geom = full_pyas.odbase_get_odbase( jun, "GEOM", 1-full_pyas.odessa_shift() )
            S = full_pyas.odbase_get_double( geom, "S", 1-full_pyas.odessa_shift() )

            ther = full_pyas.odbase_get_odbase( jun, "THER", 1-full_pyas.odessa_shift() )
            v = self.get_values( ther )
            ...
            vol_donnorg_name = full_pyas.odbase_get_string( jun, donnorg, 1-full_pyas.odessa_shift() )
            vol_donnorg = full_pyas.odbase_get_odbase( circuit, "VOLUME", self.get_index( circuit, "VOLUME", vol_donnorg_name ) )
            vol_donnorg_ther =  full_pyas.odbase_get_odbase( vol_donnorg, "THER", 1-full_pyas.odessa_shift() )
            ...
            if donnorg != donnorl:
                vol_donnorl_name = full_pyas.odbase_get_string( jun, donnorl, 1-full_pyas.odessa_shift() )
                vol_donnorl = full_pyas.odbase_get_odbase( circuit, "VOLUME", self.get_index( circuit, "VOLUME", vol_donnorl_name ) )
                vol_donnorl_ther =  full_pyas.odbase_get_odbase( vol_donnorl, "THER", 1-full_pyas.odessa_shift() )
        ...
        for gv in gvm.GVS:
            ...
            for index, gv_path in enumerate(self.gv_output_paths[gv]):
                ...
                if isinstance(gv_path, fluent.path.R1ElementPath):
                    ...
                    self.log(f"Putting value {value} at coord {gv_path.coord[0]} in tab of size "+str(full_pyas.odr1_size(tab)))
                    full_pyas.odr1_put(tab,1-full_pyas.odessa_shift(),self.time_end)
                    full_pyas.odr1_put(tab,gv_path.coord[0]+1-full_pyas.odessa_shift(),value)
    ...
    def force_restart(self):
        restart =    full_pyas.odbase_init() # Force CESAR reinitialisation
        full_pyas.odbase_insert_odbase( pyas.root_database(), "RESTART", restart, 1-full_pyas.odessa_shift() )
    ...
    def terminate_cesar_computation(self):
        ...
        primary = full_pyas.odbase_get_odbase( pyas.root_database(), "PRIMARY", 1-full_pyas.odessa_shift() )
        ...
        secondary = full_pyas.odbase_get_odbase( pyas.root_database(), "SECONDAR", 1-full_pyas.odessa_shift() )
    ...
    def main_loop(self):
        ...
        while self.icont > 0:
            ...
            while not full_pyas.boolp_value(self.success):
                ...
                self.time_beg=full_pyas.doublep_value(self.tbeg)
                self.time_end=full_pyas.doublep_value(self.tend)
