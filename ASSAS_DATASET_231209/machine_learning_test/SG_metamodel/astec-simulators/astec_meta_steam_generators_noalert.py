...
# Interaction with base
    ...
    def get_index(self, circuit, family, name):
        result = -1
        for i in range(pyas.odbase_size( circuit, family )):
            sub=pyas.odbase_get_odbase( circuit, family, i )
            if( pyas.odbase_size( sub, "NAME" ) > 0 ):
                a_name = pyas.odbase_get_string( sub, "NAME", 0 )
                if( a_name == name ) :
                    result = i
                    break
        if( result == -1 ):
    ...
    def tag(self, circuit_name, family, name, dummy_arg):
        if circuit_name is None:
            ...
            circuit = pyas.odbase_get_odbase( pyas.root_database(), circuit_name, 0 )
        index = self.get_index( circuit, family, name )
        vol = pyas.odbase_get_odbase( circuit, family, index )
        pyas.odbase_put_int( vol, "INDEX", index, 0 )
    ...
    def hide(self, circuit_name, family, name, dummy_arg):
        ...
        pyas.odbase_insert_odbase(circuit,"H"+family, vol, new_index)
    ...
    def hide_link_in_family(self, circuit, family, link, name):
        for i in range( pyas.odbase_size( circuit, family ) ):
            ...
            for j in range( pyas.odbase_size( vol, link ) ):
                jun = pyas.odbase_get_string( vol, link, j )
                if( jun == name ):
                    pyas.odbase_delete_element(vol, link, j)
                    pyas.odbase_insert_string(vol, "H"+link, name, 0)
    ...
    def restore(self, circuit, family):
        ...
        for i in range( N ):
            ...
            new_index = pyas.odbase_get_int( obj, "INDEX", 0 )
        ...
        for index,obj in sorted_list:      
            ...
            name = pyas.odbase_get_string(obj, "NAME", 0)
    ...
    def compute_flow( self, circuit, alter_jun_name, spec, vol_name, alter_vol_name ):
        ...
        jun_geom = pyas.odbase_get_odbase( jun, "GEOM", 0 )
        S = pyas.odbase_get_double( jun_geom, "S", 0 )
        if(spec=='WATER'):
            post="liq"
        else:
            post="gas"
        jun_ther = pyas.odbase_get_odbase( jun, "THER", 0 )
        Qr1 = pyas.odbase_get_odr1( jun_ther, dico_massic_flow[spec], 0 )
        Q = pyas.odr1_get( Qr1, 1 )
        ...
        donnor_vol_ther = pyas.odbase_get_odbase( donnor_vol, "THER", 0 )
        Tr1 = pyas.odbase_get_odr1( donnor_vol_ther, "T_"+post, 0 )
        T = pyas.odr1_get( Tr1, 1 )
        Pr1 = pyas.odbase_get_odr1( donnor_vol_ther, "P", 0 )
        P = pyas.odr1_get( Pr1, 1 )
    ...
    def transfer_from_vol_ther_to_connecti_ther( self, volume_ther, connecti_ther, key:str ):
        ...
        tab = pyas.odbase_get_odr1( volume_ther, key, 0 )
        pyas.odbase_put_odr1( connecti_ther, key, tab, 0 )
        self.log("Put "+key+" = "+str(pyas.odr1_get( tab, 1 )))
    
    def open_and_feed_connecti(self, conn, init):
        

        connecti = self.get( pyas.root_database(), "CONNECTI", conn )
        pyas.odbase_put_string( connecti, 'STAT', 'ON', 0 )

        vol_name = pyas.odbase_get_string( connecti, "VOLUME", 0 )
        circuit_name = pyas.odbase_get_string( connecti, "TO", 0 )
        if( circuit_name=='USER' ): circuit_name = pyas.odbase_get_string( connecti, "FROM", 0 )
        circuit = pyas.odbase_get_odbase( pyas.root_database(), circuit_name, 0 )
        
        private = pyas.odbase_get_odbase( connecti, "PRIVATE", 0 )
        alter_vol_name = pyas.odbase_get_string( private, "VOLUME", 0 )
        alter_jun_name = pyas.odbase_get_string( private, "JUNCTION", 0 )
        connecti_type = pyas.odbase_get_string( connecti, "TYPE", 0 )

        self.log( "\n Open and feed "+conn+" of type "+ connecti_type )
        
        # Connecti of type SOURCE
        if( connecti_type=="SOURCE" ):
                for i in range( pyas.odbase_size( connecti, "SOURCE" ) ):
                    source = pyas.odbase_get_odbase( connecti, "SOURCE", i )
                    spec = pyas.odbase_get_string( source, "SPEC", 0 )
                    Q, T, P = self.compute_flow( circuit, alter_jun_name, spec, vol_name, alter_vol_name )
                    if init:
                        tab = pyas.odr1_init()
                        pyas.odr1_put( tab, 0, self.time_beg )
                        pyas.odr1_put( tab, 1, Q )
                        pyas.odr1_put( tab, 2, T )
                        pyas.odr1_put( tab, 3, P )
                        pyas.odbase_put_odr1( source, "FLOW", tab, 0 )
                    else:
                        tab = pyas.odbase_get_odr1( source, "FLOW", 0 )
                        pyas.odr1_put( tab, 4, self.time_end )
                        pyas.odr1_put( tab, 5, Q )
                        pyas.odr1_put( tab, 6, T )
                        pyas.odr1_put( tab, 7, P )
                    pyas.odbase_put_odr1( source, "FLOW", tab, 0 )
                    
        elif( connecti_type=="BCPRESS" ):
            junction = self.get( circuit, "JUNCTION", alter_jun_name )
            junction_ther = pyas.odbase_get_odbase( junction, "THER", 0 )
            volume = self.get( circuit, "VOLUME", alter_vol_name )
            volume_ther = pyas.odbase_get_odbase( volume, "THER", 0 )
            connecti_ther = pyas.odbase_get_odbase( connecti, "THER", 0 )
    ...
    def transfer_from_connecti_ther_to_junction_ther( self, connecti_ther, junction_ther, key ):
        ...
        tab = pyas.odbase_get_odr1( connecti_ther, key, 0 )
        pyas.odbase_put_odr1( junction_ther, key, tab, 0 )
        self.log("Get "+key+" = "+str(pyas.odr1_get( tab, 1 )))
        
    def close_connecti(self, conn):
        
        self.log( "\n Close "+conn )
        connecti = self.get( pyas.root_database(), "CONNECTI", conn )
        pyas.odbase_put_string( connecti, 'STAT', 'OFF', 0 )
        connecti_type = pyas.odbase_get_string( connecti, "TYPE", 0 )
    ...
    def extract_data(self, source, dest ):
        ...
        for i in range( pyas.odbase_family_number(source) ):
    ...
    def extract(self, circuit_name, family, name, reduced):
        ...
        if circuit_name is None:
            ...
            circuit = pyas.odbase_get_odbase( pyas.root_database(), circuit_name, 0 )
            reduced_circuit = pyas.odbase_get_odbase( reduced, circuit_name, 0 )
    ...
    def put_values(self,base,time,values):
        for v in values:
            tab=pyas.odr1_init()
            pyas.odr1_put(tab,0,time)
            pyas.odr1_put(tab,1,values[v])
            pyas.odbase_put_odr1(base,v,tab,0 )
    ...
    def get_values(self,base):
        ...
        for i in range( pyas.odbase_family_number(base) ):
            ...
            if( ftyp==pyas.od_r0 ):
                r[fname]=pyas.odbase_get_double(base,fname,0)
            elif( ftyp==pyas.od_r1 ):
                r[fname]=pyas.odr1_get( pyas.odbase_get_odr1(base,fname,0), 1 )
            else:
    ...
    def complete_missing_data_from_metamodelled_ones(self, circuit_name, family, name, dummy_arg):
        ...
        if family=="VOLUME":
            vol = obj
            computed={}
            geom = pyas.odbase_get_odbase( vol, "GEOM", 0 )
            volume = pyas.odbase_get_double( geom, "V", 0 )
            
            ther = pyas.odbase_get_odbase( vol, "THER", 0 )
            ...
            for j in range( N ):
                ...
                juncther = pyas.odbase_get_odbase( junc, "THER", 0 )
                Vgr1 = pyas.odbase_get_odr1( juncther, "v_gas", 0 )
                Vg = pyas.odr1_get( Vgr1, 1 )
                Vlr1 = pyas.odbase_get_odr1( juncther, "v_liq", 0 )
                Vl = pyas.odr1_get( Vlr1, 1 )
                Vl_bar = Vl_bar + Vl / N
                Vg_bar = Vg_bar + Vg / N
        ...
        if family=="JUNCTION":
            computed={}
            jun = obj
            geom = pyas.odbase_get_odbase( jun, "GEOM", 0 )
            S = pyas.odbase_get_double( geom, "S", 0 )
            
            ther = pyas.odbase_get_odbase( jun, "THER", 0 )
            v = self.get_values( ther )
            ...
            vol_donnorg_name = pyas.odbase_get_string( jun, donnorg, 0 )
            vol_donnorg = pyas.odbase_get_odbase( circuit, "VOLUME", self.get_index( circuit, "VOLUME", vol_donnorg_name ) )
            vol_donnorg_ther =  pyas.odbase_get_odbase( vol_donnorg, "THER", 0 )
            ...
            if donnorg != donnorl:
                vol_donnorl_name = pyas.odbase_get_string( jun, donnorl, 0 )
                vol_donnorl = pyas.odbase_get_odbase( circuit, "VOLUME", self.get_index( circuit, "VOLUME", vol_donnorl_name ) )
                vol_donnorl_ther =  pyas.odbase_get_odbase( vol_donnorl, "THER", 0 )
        ...
        for gv in gvm.GVS:
            ...
            for index, gv_path in enumerate(self.gv_output_paths[gv]):
                ...
                if isinstance(gv_path, fluent.path.R1ElementPath):
                    ...
                    pyas.odr1_put(tab,0,self.time_end)
                    pyas.odr1_put(tab,gv_path.coord[0],value)
    ...
    def force_restart(self):
        ...
        pyas.odbase_insert_odbase( pyas.root_database(), "RESTART", restart, 0 )
    ...
    def terminate_cesar_computation(self):
        ...
        primary = pyas.odbase_get_odbase( pyas.root_database(), "PRIMARY", 0 )
        ...
        secondary = pyas.odbase_get_odbase( pyas.root_database(), "SECONDAR", 0 )
