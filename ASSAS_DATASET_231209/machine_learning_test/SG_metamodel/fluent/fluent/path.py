    ...
    def exists_from(self, base: cache.CachedOptOdbase) -> bool:
        if self.parent.exists_from(base):
            ...
            if pyas.odbase_type(parent, self.name) == self.typ:
                return self.pos < pyas.odbase_size(parent, self.name)
            else:
                return False
        else:
            return False
...
class ArrayElementPath(typing.Generic[flod.X, flod.AT, flod.C], Path[flod.X]):
    ...
    def exists_from(self, base: cache.CachedOptOdbase) -> bool:
        if self.parent.exists_from(base):
            parent = self.parent.get_from(base)
            if isinstance(self, I1ElementPath):
                return self.coord[0] < pyas.odi1_size(parent)
            elif isinstance(self, I2ElementPath):
                return self.coord[0] < pyas.odi2_card(parent, 0) \
                    and self.coord[1] < pyas.odi2_card(parent, 1)
            elif isinstance(self, R1ElementPath):
                return self.coord[0] < pyas.odr1_size(parent)
            elif isinstance(self, R2ElementPath):
                return self.coord[0] < pyas.odr2_card(parent, 0) \
                    and self.coord[1] < pyas.odr2_card(parent, 1)
            elif isinstance(self, R3ElementPath):
                return self.coord[0] < pyas.odr3_card(parent, 0) \
                    and self.coord[1] < pyas.odr3_card(parent, 1) \
                    and self.coord[2] < pyas.odr3_card(parent, 2)
            elif isinstance(self, C1ElementPath):
                return self.coord[0] < pyas.odc1_size(parent)
            else:
