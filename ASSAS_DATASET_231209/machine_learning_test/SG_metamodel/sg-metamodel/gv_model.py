...
def generate_sensor_paths(datas: Iterable[ObjectMetadata], mapping: NameMapping):
    ...
    for data in datas:
        ...
        yield ROOT.sensor(index).child_float("value", 0), PathMetadata("value", data, True, False)
...
def generate_connecti_paths(datas: Iterable[ObjectMetadata], mapping: NameMapping):
    ...
    for data in datas:
        ...
        yield connecti.child_str("STAT", 0), PathMetadata("STAT", data, True, False)
        yield connecti.child_float("SECT", 0), PathMetadata("STAT", data, True, False)
        flow = connecti.child_base("SOURCE", 0).child_r1("FLOW", 0)
        yield flow.child_element(1), PathMetadata("FLOW:MASS", data, True, True)
        yield flow.child_element(2), PathMetadata("FLOW:ENERGY", data, True, True)
...
def generate_primary_volume_paths(datas: Iterable[ObjectMetadata], mapping: NameMapping):
    ...
    for data in datas:
        ...
        ther = volume.ther()
        yield ther.child_r1("P_h2", 0,).child_element(1), PathMetadata("P_h2", data, True, True)
#        yield ther.child_r1("P_bho2", 0,).child_element(1), PathMetadata("P_bho2", data, True, True)
#        yield ther.child_r1("P_co2", 0,).child_element(1), PathMetadata("P_co2", data, True, True)
        yield ther.child_r1("P_steam", 0,).child_element(1), PathMetadata("P_steam", data, True, True)
        yield ther.child_r1("T_gas", 0,).child_element(1), PathMetadata("T_gas", data, True, True)
        yield ther.child_r1("T_liq", 0,).child_element(1), PathMetadata("T_liq", data, True, True)
        yield ther.child_r1("x_alfa", 0,).child_element(1), PathMetadata("x_alfa", data, True, True)
...
#        yield ther.child_r1("P_co2", 0,).child_element(1), PathMetadata("P_co2", data, True, True)
        ...
        if HOT_BOX_PREFIX in data.name:
            yield ther.child_r1("x_alfa1", 0,).child_element(1), PathMetadata("x_alfa1", data, True, True)
...
def generate_primary_junction_paths(datas: Iterable[ObjectMetadata], mapping: NameMapping):
    ...
    for data in datas:
        ...
        ther = junction.ther(0)
        yield ther.child_r1("v_gas", 0,).child_element(1), PathMetadata("v_gas", data, True, True)
        yield ther.child_r1("v_liq", 0,).child_element(1), PathMetadata("v_liq", data, True, True)
...
def generate_primary_wall_paths(datas: Iterable[ObjectMetadata], mapping: NameMapping):
    ...
    for data in datas:
        ...
        for ther_index in [0, 1]:
            yield wall.ther(ther_index).child_r1("T_wall", 0,).child_element(1), PathMetadata(f"T_wall {ther_index}", data, True, True)
        yield wall.child_float("fp_power", 0), PathMetadata("fp_power", data, True, False)
...
def get_gv_model_metadata(base):
    ...
    for index in range(ROOT.size_from("CONNECTI", base)):
        connecti_names[index] = ROOT.connecti(index).name_from(base)

    for index in range(ROOT.size_from("SENSOR", base)):
