    ...
    for i in range( pyas.odbase_family_number(base) ):
        ...
        for j in range( fnumber ):
            ...
            if( ftype==pyas.od_base ):
                ...
                for k in range( pyas.odbase_family_number(sub_base) ):
                    fn = pyas.odbase_name( sub_base, k ).strip()
                    if fn in indexes:
                        sub_path = "("
                        if( fn != "NAME" ) : sub_path = sub_path + fn + "="
                        sub_path = sub_path + pyas.odbase_get_string( sub_base, fn, 0 ).strip()
              ...
              if( ftype==pyas.od_r0 ):
                ...
                if fname in DUO:
                  ...
                  yield ( nextpath + "_t[1]", pyas.odr1_get( tab, 1 ) )
                ...
                for k in range( pyas.odrg_size( tab ) ):
...
def read_binary_nearby( bin, time):
  ...
  seq = pyas.odbase_get_odbase(B,'SEQUENCE',0)
  if(seq is None):
    die('read_binary_nearby '+bin+' '+str(time)+' no SEQUENCE read')
  t = pyas.odbase_get_double(seq,'TIME',0)
  dt = pyas.odbase_get_double(seq,'STEP',0)
