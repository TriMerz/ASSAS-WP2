...
if os.path.exists(cache_file):
   ...
   data = {'time': elapsed_time[0:]} # !! the DataFrame begins with the second converged solution [1:] !!
   for i in range(varprim.shape[1]):
      data[f'var{i}'] = varprim[:, i]
...
df.to_csv('init.csv', index=True)

df1 = pd.read_csv('init.csv')
