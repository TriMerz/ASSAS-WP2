...
model.add(Conv2D(filters=2233, kernel_size=(5, 5), activation='gelu', padding='same'))
model.add(MaxPool2D(pool_size=(1, 2233)))
model.add(Reshape((10, -1)))
model.add(Conv1D(filters=WINDOW_SIZE, kernel_size=1, activation='relu'))
model.add(LSTM(256, activation='relu', return_sequences=True))
model.add(LSTM(64, activation='gelu', recurrent_activation='sigmoid', return_sequences=True))
model.add(LSTM(128, activation='relu', return_sequences=True))
model.add(Flatten())
model.add(Dense(512, 'swish', kernel_regularizer=tf.keras.regularizers.l2(0.01)))
model.add(Dense(512, activation='relu', kernel_regularizer=tf.keras.regularizers.l1_l2(l1=0.01, l2=0.01)))
...
model.save_weights('less_2D1DConvlstm_model.weights.h5')
model.load_weights('less_2D1DConvlstm_model.weights.h5')
