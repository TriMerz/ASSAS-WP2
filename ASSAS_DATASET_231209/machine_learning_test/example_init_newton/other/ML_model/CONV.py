...
print("\n", "Shape of the Windowed DataBase: ", X.shape, "\n", "Shape of the label: ",y.shape) # X.shape=(99875, 10, 2234) y.shape=(99875, 2233)
...
# ========== ML model.def ========== (TOO HEAVY)
# model_convLSTM=Sequential()
# model_convLSTM.add(InputLayer((10, 1, 2234, 1)))
# model_convLSTM.add(ConvLSTM2D(64, (3, 3), activation='tanh', recurrent_activation='sigmoid', return_sequences=True, padding='same'))
# model_convLSTM.add(BatchNormalization())
# model_convLSTM.add(Flatten())
# model_convLSTM.add(Reshape((-1, 64))) 
...
model_convLSTM = Sequential()
model_convLSTM.add(InputLayer((10, 1, 2234, 1)))
model_convLSTM.add(ConvLSTM2D(32, (3, 3), activation='tanh', recurrent_activation='sigmoid', return_sequences=True, padding='same'))
model_convLSTM.add(Flatten())
model_convLSTM.add(Dense(64, activation='relu'))
model_convLSTM.add(Dense(2233, activation='linear'))
model_convLSTM.summary()

history_cb = tf.keras.callbacks.History()
early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)
model_convLSTM.compile(optimizer=Adam(learning_rate=0.0001),
              loss=MeanSquaredError(), metrics=['mae', 'mse'])
EPOCHS = 100
history = model_convLSTM.fit(X_train_reshaped, y_train, validation_data=(X_val_reshaped, y_val), epochs=EPOCHS, callbacks=[history_cb, early_stopping])
model_convLSTM.save_weights('convLSTM_model.weights.h5')

model_convLSTM.load_weights('convLSTM_model.weights.h5')
predictions = model_convLSTM.predict(X_test_reshaped)
