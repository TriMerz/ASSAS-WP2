...
# ========== ML model.def ==========
model = Sequential()
model.add(Input(shape=(WINDOW_SIZE, 2234, 1)))
model.add(Conv2D(filters=1691, kernel_size=(6, 6), activation='relu', padding='same'))
model.add(MaxPool2D(pool_size=(1, 1691)))
model.add(Conv2D(filters=4, kernel_size=(5, 5), activation='relu', padding='same'))
model.add(Reshape((10, -1)))
model.add(Conv1D(filters=WINDOW_SIZE, kernel_size=1, activation='relu'))
model.add(LSTM(64, activation='relu', return_sequences=True))
model.add(LSTM(64, activation='relu', recurrent_activation='sigmoid', return_sequences=True))
model.add(LSTM(64, activation='relu', return_sequences=True))
model.add(Flatten())
model.add(Dense(128, 'swish', kernel_regularizer=tf.keras.regularizers.l2(0.01)))
...
#model = Sequential()
#model.add(Input(shape=(WINDOW_SIZE, 2234, 1)))
#model.add(Conv2D(filters=32, kernel_size=(6, 6), activation='relu', padding='same'))
...
# model = Sequential()
# model.add(Input(shape=(WINDOW_SIZE, 2234, 1)))
# model.add(Conv2D(filters=2233, kernel_size=(5, 5), activation='relu', padding='same'))
# model.add(MaxPool2D(pool_size=(2, 2), padding='same'))
# model.add(Conv2D(filters=2233, kernel_size=(1, 1), activation='relu', padding='same'))
# model.add(Conv1D(filters=10, kernel_size=10, activation='relu'))
# model.add(LSTM(64, activation='relu', return_sequences=True))
# model.add(LSTM(64, activation='relu', recurrent_activation='sigmoid', return_sequences=True))
# model.add(LSTM(64, activation='relu', return_sequences=True))
...
model.save_weights('2D1DConvlstm_model.weights.h5')
model.load_weights('2D1DConvlstm_model.weights.h5')
