...
def objective(trial):
    ...
    for i in range(num_lstm_layers):
        units = trial.suggest_int(f'lstm_units_{i}', 1, 4466)
        activation = trial.suggest_categorical(f'lstm_activation_{i}', ['tanh', 'relu', 'swish', 'sigmoid'])
        model.add(LSTM(units, activation=activation, return_sequences=(i != num_lstm_layers - 1)))
        # == BatchNormalization ==
        if trial.suggest_categorical(f'batch_normalization_{i}', [True, False]):
            model.add(BatchNormalization())
    # == Dense layer ==
    num_dense_layers = trial.suggest_int('num_dense_layers', 1, 13)
    for i in range(num_dense_layers):
        units = trial.suggest_int(f'dense_units_{i}', 1, 6699)
        activation = trial.suggest_categorical(f'dense_activation_{i}', ['relu', 'swish', 'sigmoid'])
        kernel_regularizer = trial.suggest_categorical(f'dense_regularizer_{i}', [None, 'l1', 'l2', 'l1_l2'])
        if kernel_regularizer == 'l1':
            reg_param = trial.suggest_float(f'l1_regularizer_{i}', 1e-5, 1e-2, log=True)
            kernel_regularizer = l1(reg_param)
        elif kernel_regularizer == 'l2':
            reg_param = trial.suggest_float(f'l2_regularizer_{i}', 1e-5, 1e-2, log=True)
            kernel_regularizer = l2(reg_param)
        elif kernel_regularizer == 'l1_l2':
            l1_reg_param = trial.suggest_float(f'l1_regularizer_{i}', 1e-5, 1e-2, log=True)
            l2_reg_param = trial.suggest_float(f'l2_regularizer_{i}', 1e-5, 1e-2, log=True)
            kernel_regularizer = l1_l2(l1=l1_reg_param, l2=l2_reg_param)
    ...
    model.compile(optimizer=Adam(learning_rate=learning_rate), loss=MeanSquaredError(), metrics=['mae', RootMeanSquaredError()])
    
    history = model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=epochs, callbacks=[history_cb, early_stopping], verbose=0)
    return history.history['val_loss'][-1]

def optuna_progress(study, trial):
    print(f"Iteration {trial.number}/{len(study.trials)} - Value: {trial.value}")
    print("Params:")
    for key, value in trial.params.items():
        print(f"    {key}: {value}")

study = optuna.create_study(direction='minimize')
study.optimize(objective, n_trials=100, callbacks=[optuna_progress])

# == Optimization RESULTS ==
print('Best trial:')
best_trial = study.best_trial
best_model = load_model(best_trial.user_attrs['model_path'])
best_model.save('model/best_optuna_model.h5')
print(f'Value: {best_trial.value}')
print('Params:')
for key, value in best_trial.params.items():
    print(f'    {key}: {value}')

model1 = load_model('model/optuna_model.h5')
predictions = model1.predict(X_test)    # data extraction for printing:   X_train[0][:3][:,1]   y_train[0][0]
