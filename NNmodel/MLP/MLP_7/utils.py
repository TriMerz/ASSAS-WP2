                ...
                if use_amp and torch.cuda.is_available():
                    with torch.amp.autocast(device_type='cuda'):
                        ...
                        if self.default:
                            ...
                            # Handle skip connections for Performance architecture
    ...
    def _validate_epoch(self, val_loader, criterion):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                # Forward pass with architecture-specific handling
                if self.default:
                    embedding = self.encoder.encode(batch)
                    reconstruction = self.decoder.decode(embedding)
                else:
            ...
            try:
                for i in range(0, len(embeddings), batch_size):
                    ...
                    if self.default:
                        ...
                        # For inverse transform, we don't have skip connections
                        # Create empty skip connections or use a simplified decoder path
